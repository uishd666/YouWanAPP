...
const WuHu = () => {
    ...
    const handleTouch = (event: any) => {
        if (scale._value < 1.1) {
            ...
            if (Math.abs(locationX - 230) <= 70 && Math.abs(locationY - 310) <= 30) {
                setSelection(dict.YaShanDiZhi);
                setShowup(true);
            }
            else if (Math.abs(locationX - 165) <= 70 && Math.abs(locationY - 65) <= 60) {
